...
| String
| Yes
| The URL where the artifact version is located. Formatted as a comma-separated list for passing multiple artifacts.
...
| `artifact-id`
| String
| The unique identifier of the artifact. If multiple artifacts are registered, this will be the ID of the first one.
  ...
  build:
    steps:
      ...
      - name: Register an artifact with the CloudBees Unify
        ...
        with:
          name: "myApp"
          url: ${{ vars.DOCKER_REGISTRY }}/myApp:${{ cloudbees.version }},${{ vars.DOCKER_REGISTRY2 }}/myApp:${{ cloudbees.version }}
          version: "1.0.0"
      - name: Print artifact ID outputs
        uses: docker://alpine:latest
        shell: sh
        run: |
            echo "Single artifact ID: ${{ steps.register.outputs.artifact-id }}"
            echo "All artifact IDs (JSON): ${{ steps.register.outputs.artifact-ids }}"
            echo "Artifact ID for first registry: ${{ fromJSON(steps.register.outputs.artifact-ids)[vars.DOCKER_REGISTRY] }}"
            echo "Artifact ID for second registry: ${{ fromJSON(steps.register.outputs.artifact-ids)[vars.DOCKER_REGISTRY2] }}"
----
--
